...
class GuidedBertSelfAttention(BertSelfAttention):
    ...
    def forward(self, *args, **kwargs):
        ...
        # 透传所有参数给父类，解决 TypeError: encoder_attention_mask 等问题
        outputs = super().forward(*args, **kwargs)

        # outputs: (context_layer, attention_probs, past_key_value)
        attn_probs = outputs[1]

        if gold_mask is not None:
            # gold_mask: (batch, seq) -> (batch, 1, 1, seq)
            gm = gold_mask.unsqueeze(1).unsqueeze(2)
            # 引导机制：增强解释性对应的 token 权重
...
def main():
    ...
    # 显式使用 eager 模式保证自定义 SelfAttention 被调用
